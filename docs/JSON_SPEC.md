...
`sub_drop`, `transition`, `ui`. `prompt` перекрывает встроенное описание звука.
Пустой массив → набор предлагается автоматически: удар под крючок, переход на
каждой склейке, «поп» под CTA.
